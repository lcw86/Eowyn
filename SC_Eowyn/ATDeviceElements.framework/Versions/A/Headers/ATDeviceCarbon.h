...
typedef NS_ENUM(uint8_t, ATDCanOpenNMTMessage){
    
    ATDCanOpenNMT_OPERATIONAL = 1,
    ATDCanOpenNMT_STOP,
    ATDCanOpenNMT_PRE_OPERATIONAL,
    ATDCanOpenNMT_RESET_NODE,
    ATDCanOpenNMT_RESET_COMMUNICATION,
...
/**
 Command to Read SDO from CANOPen Slave Node.
 
 @param nodeID [in]  node ID
 @param index [in] SDO Index
 @param subIndex [in] SDO Sub Index
 @param data [out] Data read back, MAX 255. User might pass in a uint8_t array.
 @param actualDataRead [out] actual Data read back from SDO Slave Node.
 ...
 */
- (BOOL) canOpenSDOupLoadFromNodeID: (uint8_t)   nodeID
                           andIndex: (uint16_t)  index
                        andSubIndex: (uint8_t)   subIndex
                            andData: (uint8_t*)  data
                      andActualReadLength: (uint8_t*)  actualDataRead
                           andError: (NSError**) aError;


/**
 Command to Write  SDO to CANOPen Slave Node.
 
 @param nodeID [in] node ID
 @param index [in]  SDO Index
 @param subIndex [in]  SDO Sub Index
 @param data [in] Data to write to SDO Slave Node
 @param writelength  write length
 @param aError  nil = NO error, or error set. Error code is defined in <ATDeviceElements/ATDeviceCarbonErrors.h>
 @return YES = sucess NO = Error;
 */
- (BOOL) canOpenSDOdownLoadToNodeID: (uint8_t)   nodeID
                           andIndex: (uint16_t)  index
                        andSubIndex: (uint8_t)   subIndex
                            andData: (uint8_t*)  data
                     andWritelength: (uint16_t)  writelength
                           andError: (NSError**) aError;


- (BOOL) canOpenNetworkManagementToNodeID:(uint8_t)   nodeID
                            andNMTMessage:(ATDCanOpenNMTMessage) nmtMessage
                                 andError: (NSError**) aError;
 ...
 *
 * param uartID                        UART ID 1 to 6.
 * param writeLen                      Number of bytes to write. Max 256-32 = 224.
 * param readTerminator                UART read will stop when see this terminator. Max number of character is 32. Pass nil if don't to read
 * param actualRead                   The Actual Read only valid if readTerminator is non nil. Max it's 255 bytes
 * param data                       The Data to read and write. Size Must be at least 256 byte.
 * param uartReadTimeOutInMiliSeconds  read time out in Milio seconds
 * param aError nil = NO error, or error set. Error code is defined in <ATDeviceElements/ATDeviceCarbonErrors.h>
 * retval BOOL: YES = sucess NO = Error;
 *
 *
...
/**
 configureCANOpenWithBaudRate  If you have a Oxygen or Spectre White, you can set up your CANOpen Network

 @param baudRate baudRate for the CAN1 that use as CANOpen Master.
 @param aError nil = NO error, or error set. Error code is defined in <ATDeviceElements/ATDeviceCarbonErrors.h>
 @return YES = sucess NO = Fail;
 */
- (BOOL) configureCANOpenWithBaudRate: (uint32_t) baudRate
                             andError: (NSError**) aError;
